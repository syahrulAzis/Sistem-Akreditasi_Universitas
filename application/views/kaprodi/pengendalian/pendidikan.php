                                            ...
                                            <td>
                                                <div class="progress">
                                                    ...
                                                    if ($n > 2) {
                                                        ...
                                                        $dokOB = $this->db->count_all_results();

                                                        $this->db->like('pendidikan_id', $hp);
                                                        $this->db->like('role_id', $this->session->userdata('role_id'));
                                                        $this->db->like('status_id', 3);
                                                        $this->db->from('pendidikan_transaksi');
                                                        $dokKTS = $this->db->count_all_results();

                                                        $jumDOK = $dokOB + $dokKTS;

                                                        $this->db->like('pendidikan_id', $hp);
                                                        $this->db->like('role_id', $this->session->userdata('role_id'));
                                                        $this->db->like('log_pengendalian', $this->session->userdata('id'));
                                                        // $this->db->like('status_id', 0);
                                                        $this->db->from('pendidikan_transaksi');
                                                        $isi_dok = $this->db->count_all_results();

                                                        if ($isi_dok > 0) {
                                                            $persen = $isi_dok / $jumDOK * 100;
                                                ...
                                                if ($p['is_active_pendidikan'] > 3) {
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton6">
                                                            <a class="dropdown-item" href="' . site_url('Kaprodi/HasilPengendalian/' . $p['id_pendidikan']) . '?>">Hasil Pengendalian</a>
                                                            <a class="dropdown-item" href="' . site_url('Kaprodi/MutuPengendalian/' . $p['id_pendidikan']) . '?>">Mutu</a>
                                                        </div>
                                                    </div>';
                                                    ...
                                                    <div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuIconButton6">
                                                        <a class="dropdown-item" href="' . site_url('Kaprodi/DetailPendidikanPengendalian/' . $p['id_pendidikan']) . '?>">Detail</a>
                                                        </div>
                                                    </div>';
                                                } else {
                                                    echo '
